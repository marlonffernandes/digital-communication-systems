...
rolloff=1;        % Fator de rolloff dos filtros Tx e Rx
...
[bn,an]=butter(2,2*1500/Fs);          % Filtro passa-baixas de largura de banda fc
%sinal_rx=filter(bn,an,sinal_tx_filtrado);
...
xlabel('Frequência (kHz)');                %do filtro que simula o canal
ylabel('Ganho (dB)');
...
grid; xlabel('Frequência (kHz)'); ylabel('dBm/Hz');
xlim([-(Fs/4)/1000 (Fs/4)/1000]);
title('Sinal na Saída do Filtro Casado')
